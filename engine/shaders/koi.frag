...
float evaluateCaustics(vec2 p, float t) {
    vec2 p1 = p * 14.0 + vec2(t * 0.35, -t * 0.25);
    vec2 p2 = p * 18.0 - vec2(-t * 0.25, t * 0.40);
...
void main() {
    ...
    float waveDx = sin((p.x + eps) * 16.0 * 1.8 + t * 2.2) * cos(p.y * 16.0 * 1.5 - t * 1.8) * 0.006;
    float waveDy = sin(p.x * 16.0 * 1.8 + t * 2.2) * cos((p.y + eps) * 16.0 * 1.5 - t * 1.8) * 0.006;
    vec2 waterGradient = vec2(waveDx - waveHeight, waveDy - waveHeight) / eps;
    vec3 waterNormal = normalize(vec3(-waterGradient.x * 1.5, -waterGradient.y * 1.5, 0.45));

    // Refracted UV for looking through the water volume onto the pond bed
    vec2 refrUV = p + waterNormal.xy * 0.035;
